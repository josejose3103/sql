...
sql = "SELECT 農家名, sum(金額) FROM 平成１５年 WHERE 日付 BETWEEN #2021/12/23# AND #2022/01/10# group by 農家名"
#sql = "SELECT DATEPART(YEAR, 日付), sum(金額) FROM 平成１５年 GROUP BY DATEPART(YEAR, 日付)"
     


#sql = "SELECT sum(金額) FROM 平成１５年 WHERE 日付 BETWEEN #2021/12/23# AND #2022/01/10#"
