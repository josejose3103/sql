...
tax = total_amount - tax_excluded  # 消費税額（総額 - 税抜き）
...
c.drawString(150, height - 110, "病名")
c.drawString(350, height - 110, "金額（税込）")
...
for index, row in df.iterrows():
    ...
    c.drawString(150, y_position, row["病名"])  # 日本語フォント適用済み
    c.drawString(350, y_position, f"{row['金額']} 円")  # 金額（税込）
    y_position -= 20  # 行間

# ✅ 内税方式で金額を表示（税抜き・消費税・税込み）
y_position -= 30
